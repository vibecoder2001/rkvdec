    ...
    struct v4l2_ctrl_h264_decode_params *decode)
{
    ...
    if (st == V4L2_H264_SLICE_TYPE_P || st == V4L2_H264_SLICE_TYPE_SP ||
        ...
        /* ref_pic_list_modification (H.264 7.3.3.1) — walk so post-
         * header bits stay aligned.  We don't surface modifications
         * to the regbuilder; they primarily steer the codec's RefPicList
         * derivation, which the rkvdec hardware re-runs from the slice
         * header bitstream itself.  For B-pyramid streams generated by
         * ffmpeg testsrc2 the modification flag is typically 0. */
        if (st != V4L2_H264_SLICE_TYPE_I && st != V4L2_H264_SLICE_TYPE_SI) {
            ...
            if (mod_l0) {
                while (1) {
                    ...
                    if (mod_op == 3) break;
                    (void)br_ue(br);   /* abs_diff_pic_num_minus1 / ltidx */
                    if (br_eof(br)) break;
                }
            }
            if (st == V4L2_H264_SLICE_TYPE_B) {
    ...
    /* dec_ref_pic_marking (H.264 7.3.3.1) — walk so following bits
     * (cabac_init_idc, slice_qp_delta) stay aligned.  We don't surface
     * MMCO ops to the DPB layer in this scope; for our test stream
     * (sliding-window, no long-term refs) the operations are no-ops
     * to the codec's reg builder. */
    if (nal_ref_idc != 0) {
        if (nal_unit_type == 5) {
            (void)br_u1(br);                            /* no_output_of_prior_pics */
            (void)br_u1(br);                            /* long_term_reference_flag */
        } else {
            ...
            if (adaptive) {
                while (1) {
                    ...
                    if (op == 0) break;
                    if (op == 1 || op == 3) (void)br_ue(br); /* difference_of_pic_nums_minus1 */
                    if (op == 2)            (void)br_ue(br); /* long_term_pic_num */
                    if (op == 3 || op == 6) (void)br_ue(br); /* long_term_frame_idx */
                    if (op == 4)            (void)br_ue(br); /* max_long_term_frame_idx_plus1 */
