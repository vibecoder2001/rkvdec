/*
 * regbuilder_av1_reg.h — VdpuAv1d register-set bitfield definitions
 *                        for the RK3588 AV1 hardware decoder.
 *
 * This file is vendored from rockchip-linux/mpp at:
 *   mpp/hal/vpu/av1d/hal_av1d_vdpu_reg.h
 *
 * Original copyright + license header preserved below.  We use this
 * header as a factual hardware-register description (the bitfield
 * layouts are properties of the silicon) and link against our own
 * regbuilder_av1.cpp implementation that fills these structs.
 *
 * Per the project's licensing memory, only this single MPP header is
 * lifted verbatim — all logic that consumes it is clean-room.
 *
...
#ifndef RKMPP_REGBUILDER_AV1_REG_H
#define RKMPP_REGBUILDER_AV1_REG_H

#include <stdint.h>

/* The original MPP header used "RK_U32" everywhere via mpp_device.h.
 * Map it onto our standard fixed-width type so the struct layout stays
 * bit-identical without dragging in MPP's headers. */
#ifndef RK_U32
...
#endif /* RKMPP_REGBUILDER_AV1_REG_H */
